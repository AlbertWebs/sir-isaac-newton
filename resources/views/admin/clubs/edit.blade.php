...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-teal-600 to-cyan-600 px-6 py-4">
            ...
            <p class="text-teal-100 text-sm mt-1">Update club information</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.clubs.update', $club->id) }}" class="p-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="group">
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $club->code) }}" required
                        class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-lg bg-white"
                        placeholder="e.g., FC">
                    @error('code')
                        <p class="text-red-500 text-xs mt-1.5 flex items-center">
                ...
                <div class="group">
                    ...
                    <select id="type" name="type" required
                        ...
                        @foreach($clubTypes as $type)
                            <option value="{{ $type }}" {{ old('type', $club->type) === $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs mt-1.5 flex items-center">
                ...
                <div class="group">
                    ...
                    <input type="number" id="max_members" name="max_members" value="{{ old('max_members', $club->max_members) }}" min="1"
                        class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-lg bg-white"
                ...
                <div class="group">
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="suspended" {{ old('status', $club->status) === 'suspended' ? 'selected' : '' }}>Suspended</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1.5 flex items-center">
                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">

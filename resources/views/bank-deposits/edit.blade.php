...
<div class="max-w-2xl mx-auto" x-data="bankDepositForm()">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form method="POST" action="{{ route('admin.bank-deposits.update', $bankDeposit->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('admin.bank-deposits.show', $bankDeposit->id) }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                    Cancel
                </a>
                <button 
                    type="submit" 
...
function bankDepositForm() {
    return {
        sourceAccount: '{{ $bankDeposit->source_account }}',
        amount: '{{ old('amount', $bankDeposit->amount) }}',
        balanceMessage: '',
        hasBalance: false,
        loading: false,
        ...
        async fetchBalance() {
            ...
            try {
                ...
                if (data.has_balance) {
                    ...
                    if (!this.amount || this.amount === '{{ old('amount', $bankDeposit->amount) }}') {
